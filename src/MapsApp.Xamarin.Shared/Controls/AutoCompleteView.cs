...
namespace Esri.ArcGISRuntime.ExampleApps.MapsApp.Xamarin.Controls
{
    ...
    public class AutoCompleteView : ContentView
    {
        ...
        /// </summary>
        public static readonly BindableProperty SuggestionsProperty = BindableProperty.Create("Suggestions", typeof(IEnumerable), typeof(AutoCompleteView), null, BindingMode.TwoWay, propertyChanged: StaticSuggestionsValueChanged);

        /// <summary>
        /// Occurs when [suggestions value changes]
        /// </summary>
        private static void StaticSuggestionsValueChanged(BindableObject bindable, object oldValue, object newValue)
        {
            if (newValue != null)
            {
                var nv = newValue as ObservableCollection<string>;

                var control = bindable as AutoCompleteView;
                control._availableSuggestions.Clear();
                foreach (var i in nv)
                {
                    control._availableSuggestions.Add(i);
                }
        ...
        private readonly SearchBar _entText;
        ...
        public AutoCompleteView()
        {
            ...
            _entText.SearchButtonPressed += (s, e) =>
            {
            ...
            _lstSuggestions.ItemSelected += (s, e) =>
            {
                ...
                // Update SelectedItem and fire change event
                OnSelectedItemChanged(e.SelectedItem);

                // Get text for selected item
                string selectedItemText = null;
                var item = e.SelectedItem;
                if (!string.IsNullOrWhiteSpace(SuggestionTextPropertyName))
                {
                    try
                    {
                        var propertyInfo = item?.GetType()?.GetTypeInfo()?.GetDeclaredProperty(
                            SuggestionTextPropertyName);
                        selectedItemText = propertyInfo?.GetValue(item) as string;
                    }
                    catch { }
                }
                ...
                // Execute search command if specified
        ...
        /// <summary>
        /// Gets or sets the name of the property on the suggestion object containing the text to be displayed
        /// </summary>
        public string SuggestionTextPropertyName { get; set; }
        ...
        private static void TextValueChanged(BindableObject obj, string oldPlaceHolderValue, string newPlaceHolderValue)
        {
            ...
            if (newPlaceHolderValue != control._entText.Text)
            {
                control._entText.Text = newPlaceHolderValue;
            }

            if ((control != null && string.IsNullOrWhiteSpace(newPlaceHolderValue)))
            {
                // Hide suggestions
                ...
                // Toggle whether search button is enabled
